...
const TreeNode = struct {
    const Node = std.SinglyLinkedList(struct { branch: *TreeNode, count: u8 }).Node;
    children: ?*Node = null,
    name: []const u8,
    visited: bool,

    pub fn addChild(self: *TreeNode, child: *TreeNode, child_count: u8, allocator: *std.mem.Allocator) !void {
        var child_node = try allocator.create(Node);
        child_node.data.branch = child;
        child_node.data.count = child_count;
        if (self.children) |sub_bags| {
            sub_bags.insertAfter(child_node);
        } else {
    ...
    pub fn countDescendents(self: *TreeNode) usize {
        ...
        while (current_node) |value| {
            current_node = value.next;
            if (value.data.branch.visited) continue;
            count += 1 + value.data.branch.countDescendents();
        }
        self.visited = true;
        return count;
    }

    pub fn countDescendentsAmount(self: *TreeNode) usize {
        var count: usize = 0;
        var current_node = self.children;
        while (current_node) |value| {
            current_node = value.next;
            if (value.data.branch.visited) continue;
            count += @intCast(u64, value.data.count) * (1 + value.data.branch.countDescendentsAmount());
        }
        stdout.print("{} {}\n", .{ self.name, count }) catch |_| {};
        //self.visited = true;
        return count;
    }
};
...
pub fn buildTree(file: []const u8, bags_map: *std.StringHashMap(*TreeNode), allocator: *std.mem.Allocator) !void {
    ...
    while (line_iterator.next()) |line| {
        ...
        while (comma_iterator.next()) |child_bag| {
            ...
            if (std.mem.eql(u8, child_string, "no other bag")) break;
            ...
            try child.addChild(parent, 1, allocator);
        }
    }
}
...
pub fn buildTreeBackwards(file: []const u8, bags_map: *std.StringHashMap(*TreeNode), allocator: *std.mem.Allocator) !void {
    var line_iterator = getDelimIterator("\n", file);
    while (line_iterator.next()) |line| {
        var parent_child = splitString(" contain ", line) orelse return error.NoContain;
        var parent_string = stripIrrelevant(parent_child[0]);
        ...
        var comma_iterator = getDelimIterator(", ", parent_child[1][0 .. parent_child[1].len - 1]);
        while (comma_iterator.next()) |child_bag| {
            if (std.mem.eql(u8, child_bag, "no other bags")) break;
            var child_string = stripIrrelevant(child_bag);
            var child_count = child_bag[0] - '0';
            var child: *TreeNode = undefined;
            if (bags_map.get(child_string)) |child_value| {
                child = child_value;
            } else {
            ...
            try parent.addChild(child, child_count, allocator);
        }
    }
}
...
test "count nested" {
    var file =
        \\shiny gold bags contain 2 dark red bags, 5 bright pink bags.
        \\dark red bags contain 2 dark orange bags.
        \\dark orange bags contain 2 dark yellow bags.
        \\striped bags contain 3 shiny gold bags.
    ...
    try buildTreeBackwards(file, &bags_map, std.testing.allocator);
    var gold_node = bags_map.get("shiny gold bag") orelse unreachable;
    std.testing.expect(gold_node.countDescendentsAmount() == 131);
}
...
pub fn main() !void {
    ...
    try buildTreeBackwards(file_buffer[0 .. file_length - 1], &bags_map, allocator);
    var gold_node = bags_map.get("shiny gold bag") orelse return error.NoShinyGold;
    try stdout.print("{}\n", .{gold_node.countDescendentsAmount()});
}
